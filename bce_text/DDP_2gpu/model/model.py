...
class Model(torch.nn.Module):
    ...
    def forward(self, sample_items, log_mask, device):
        if self.use_modal:
        ...
        prec_vec = self.user_encoder(input_logs_embs, log_mask, device)
        pos_score = (prec_vec * target_pos_embs).sum(-1)
        neg_score = (prec_vec * target_neg_embs).sum(-1)
        pos_labels, neg_labels = torch.ones(pos_score.shape).to(device), torch.zeros(neg_score.shape).to(device)
