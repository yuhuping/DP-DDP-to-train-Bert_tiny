...
class BuildTrainDataset(Dataset):
    ...
    def __getitem__(self, user_id):
        seq = self.u2seq[user_id]                               # seq为用户user_id所交互的新闻集合
        seq_Len = len(seq)
        tokens_Len = seq_Len - 1
        mask_len_head = self.max_seq_len - seq_Len
        log_mask = [0] * mask_len_head + [1] * tokens_Len       # 创建一个掩码 log_mask，用于指示模型应该关注序列的哪些部分。

        sample_items = []                                       # 初始化 sample_items 列表，用于存储正样本和负样本
        padding_seq = [0] * mask_len_head + seq                 # 生成正样本 padding_seq，长度为 mask_len_head，填充值为0。
        sample_items.append(padding_seq)
        neg_items = []
        for i in range(tokens_Len):
            sam_neg = random.randint(1, self.item_num)
            while sam_neg in seq:                               # 生成负样本 neg_items 确保不与正样本重复
                sam_neg = random.randint(1, self.item_num)
