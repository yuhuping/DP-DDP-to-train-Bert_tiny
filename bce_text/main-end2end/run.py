...
def train(args, use_modal, local_rank):
    if use_modal:
        if 'roberta' in args.bert_model_load:
            ...
            bert_model_load = r"E:\IDvs.MoRec-main\pretrained_models\bert_base_uncased" # bert_model_load预训练模型的文件夹
            tokenizer = BertTokenizer.from_pretrained(bert_model_load) # 加载tokenizer.config.json、tokenizer.json
            config = BertConfig.from_pretrained(bert_model_load, output_hidden_states=True) # 加载config.json
            bert_model = BertModel.from_pretrained(bert_model_load, config=config) # 加载pytorch_model.bin
            ...
            if 'large' in args.bert_model_load:
                pooler_para = [389, 390]
                args.word_embedding_dim = 1024
        # 遍历BERT模型的所有参数。named_parameters() 返回一个生成器，包含模型中所有参数的名称和值
        for index, (name, param) in enumerate(bert_model.named_parameters()): 
            # 检查参数的索引是否小于 args.freeze_paras_before 或者是否在 pooler_para 列表中
...
if __name__ == "__main__":
    ...
    torch.cuda.set_device(0)           # 设置当前使用的GPU
    dist.init_process_group(backend='gloo')     # 初始化分布式训练环境，使用NCCL后端
    setup_seed(12345)
    gpus = torch.cuda.device_count()

    if 'modal' in args.item_tower:              # 根据item_tower参数判断是否使用模态信息，默认使用
        is_use_modal = True
        model_load = args.bert_model_load       # BERT具体模型的名称
        # 构建日志和模型保存的标签
    ...
    # 设置日志记录器，用于文件和屏幕输出
    Log_file, Log_screen = setuplogger(dir_label, log_paras, time_run, args.mode, dist.get_rank(), args.behaviors) # setuplogger来自data_utils
    Log_file.info(args)
